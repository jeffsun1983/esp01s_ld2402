...
void LD2402Component::dump_config() {
  ESP_LOGCONFIG(TAG, "LD2402 custom component");
}

void LD2402Component::flush_input() {
  while (available()) {
    read();
  }
}

void LD2402Component::send_command(uint16_t cmd, const std::vector<uint8_t> &data) {
  flush_input();
...
std::vector<uint8_t> LD2402Component::read_response(uint32_t timeout_ms) {
...
bool LD2402Component::read_parameter_internal(uint16_t param_id, uint32_t &value) {
...
bool LD2402Component::enter_config_mode() {
...
bool LD2402Component::exit_config_mode() {
  if (!config_mode_) return true;
  send_command(0x00FE, {});
  read_response(200);
  config_mode_ = false;
...
void LD2402Component::sync_all_configs() {
...
void LD2402Component::set_motion_threshold(uint8_t gate, float value_db) {
  if (gate > 15) return;
  enter_config_mode();
...
void LD2402Component::set_micro_threshold(uint8_t gate, float value_db) {
  if (gate > 15) return;
  enter_config_mode();
  uint32_t raw = db_to_raw(value_db);
...
void LD2402Component::set_max_distance(float distance_meters) {
...
void LD2402Component::set_timeout_seconds(uint16_t timeout_seconds) {
  enter_config_mode();
...
void LD2402Component::save_config() {
  send_command(0x00FD, {});
  read_response(200);
}

uint32_t LD2402Component::db_to_raw(float db) {
  return (uint32_t)(powf(10.0f, db / 10.0f));
}

float LD2402Component::raw_to_db(uint32_t raw) {
  return 10.0f * log10f(raw);
}

void LD2402Component::register_motion_number(uint8_t gate, number::Number *num) {
  motion_numbers_[gate] = num;
}
void LD2402Component::register_micro_number(uint8_t gate, number::Number *num) {
  micro_numbers_[gate] = num;
}
void LD2402Component::register_distance_number(number::Number *num) {
  distance_number_ = num;
}
void LD2402Component::register_timeout_number(number::Number *num) {
  timeout_number_ = num;
}

void LD2402Number::setup() {
  if (type_ == "motion") parent_->register_motion_number(gate_, this);
  else if (type_ == "micro") parent_->register_micro_number(gate_, this);
}

void LD2402Number::control(float value) {
